...
# ######## Flatten all data.
# depth_flat = np.reshape(fit_depth_data, np.prod(fit_depth_data.shape)) # flattened cortical depth.
# atlas_flat = np.reshape(fit_atlas_data, np.prod(fit_atlas_data.shape)) # flattened atlas
# img_flat = np.reshape(img_masked, (np.prod(img_masked.shape[:-1]), img_masked.shape[-1])) # flattened data
