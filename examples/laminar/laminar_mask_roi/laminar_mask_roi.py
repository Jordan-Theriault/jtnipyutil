def laminar_mask_roi_list(subj, depth_file, img_file, conf_file, work_dir, roi_dir, out_label,
    ...
    for roi in roi_files:
        fit_atlas = resample_img(nib.load(roi),
        ...
        nib.save(nib.Nifti1Image(fit_atlas_data, fit_atlas.affine,
                             fit_atlas.header), os.path.join(work_dir, 'ALIGN_'+roi.split('/')[-1]))
        roi_refit_files.append(os.path.join(work_dir, 'ALIGN_'+roi.split('/')[-1]))
    ...
    depth_masked = niftimask_depth.transform(depth_refit)

    # atlas may be 2d here, with 4d now on 2nd dimension
    ...
    roi_output_3 = []
    roi_countout_3 = []
    for idx, roi in enumerate(roi_refit_files):
        atlas_refit = nib.load(roi)
        ...
        assert img_masked.shape[-1] == atlas_masked.shape[-1], 'flattened image and flattened atlas are not equal on dimension -1'

        ######## Correlate cortical depth with BOLD
        roi_data = img_masked[:, [atlas_masked[:,] == 1][0][0][:]]
        roi_depth = depth_masked[0][[atlas_masked == 1][0][0][:]]
        ...
        try:
            print('loading data from roi %s into 10-category depth output' % roi.split('/')[-1])
        ...
        try:
            print('loading data from roi %s into 3-category depth output' % roi.split('/')[-1])
    ...
    ######## Save correlations
    # corr_out = pd.DataFrame({'subj':np.repeat(subj, len(roi_refit_files)),
    #                          'roi':roi_refit_files,
    ...
    # ######## Save depth 10
    depth10_out = pd.DataFrame({'subj':np.repeat(subj, len(np.repeat(roi_refit_files, 10))),
                                'roi':np.repeat([i.split('/')[-1] for i in roi_refit_files], 10),
                                'depth_cat':list(range(0,10))*len(roi_refit_files),
                                'cat_N':roi_countout_10})
    depth10_temp = pd.DataFrame(roi_output_10)
    depth10_out = depth10_out.join(depth10_temp)
    depth10_out.to_csv(os.path.join(work_dir, 'depth10_'+out_label+'_'+img_file.split('/')[-1].split('.nii.gz')[0]+'.csv'),
                 index=False, header=True)

    ######## Save depth 3
    depth3_out = pd.DataFrame({'subj':np.repeat(subj, len(np.repeat(roi_refit_files, 3))),
                                'roi':np.repeat([i.split('/')[-1] for i in roi_refit_files], 3),
...
roi_dir = root+'/data/rois' # The major difference from the atlas method is here: Specify directory that has .nii files.
out_label = 'AROMA_WM_CSF_detrend'
...
detrend_tf = True
laminar_mask_roi_list(subj, depth_file, img_file, conf_file, work_dir, roi_dir, out_label,
                          conf_names, detrend_tf)
out_label = 'NoMotion_NoTrend'
conf_names = None
detrend_tf = False
laminar_mask_roi_list(subj, depth_file, img_file, conf_file, work_dir, roi_dir, out_label,
                          conf_names, detrend_tf)
